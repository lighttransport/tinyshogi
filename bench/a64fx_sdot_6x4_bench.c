...
int main(int ac,char**av){int K=ac>1?atoi(av[1]):256,R=ac>2?atoi(av[2]):1000;if(K<=0||(K&3))return 2;
 int8_t*a=aa((size_t)M*K),*b=aa((size_t)K*N),*ap=aa((size_t)K*6),*bp=aa((size_t)K*N);int32_t*c=aa((size_t)M*N*sizeof(*c));if(!a||!b||!ap||!bp||!c)return 1;memset(c,0,(size_t)M*N*sizeof(*c));
